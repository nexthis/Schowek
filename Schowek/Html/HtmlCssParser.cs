    ...
    internal static class HtmlCssParser
    {
        ...
        private static string ParseCssSize(string styleValue, ref int nextIndex, bool mustBeNonNegative)
        {
            ...
            if (nextIndex < styleValue.Length && Char.IsDigit(styleValue[nextIndex]))
            {
                ...
                {
                    return myCssPraser.fontSizeConverter(number, styleValue);
                }
            }
        ...
        private static string ParseCssColor(string styleValue, ref int nextIndex)
        {
            ...
            if (nextIndex < styleValue.Length)
            {
                ...
                else if (styleValue.Substring(nextIndex, 3).ToLower() == "rgb")
                {
                    ...
                    string onlyValue = styleValue;
                    int start = onlyValue.LastIndexOf("(");
                    int end = onlyValue.LastIndexOf(")");
                    onlyValue = onlyValue.Substring(start + 1, end - start - 1);
                    ...
                    var colorTable = onlyValue.Split(',');
                    Color myColor = Color.FromArgb(int.Parse(colorTable[0]), int.Parse(colorTable[1]), int.Parse(colorTable[2]));

                    string hex = myColor.R.ToString("X2") + myColor.G.ToString("X2") + myColor.B.ToString("X2");
                    color = "#" + hex; // return bogus color
                }
                ...
                //MessageBox.Show(styleValue.Substring(nextIndex, 3).ToLower());
            }
        ...
        private static readonly string[] _fontWeights = new string[] { "normal", "bold", "ExtraBold", "Light", "Medium", "Medium", "Medium", "Bold", "Bold", "Bold", "ExtraBold", "ExtraBold", "ExtraBold" };
        ...
        private static void ParseCssFont(string styleValue, Hashtable localProperties)
        {
            ...
            ParseCssSize(styleValue, ref nextIndex, localProperties, "font-size", /*mustBeNonNegative:*/
                    true);
